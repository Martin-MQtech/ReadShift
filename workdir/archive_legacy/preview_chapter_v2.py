...
client = OpenAI(api_key=z['options']['apiKey'], base_url=z['options']['baseURL'], timeout=45, max_retries=2)
...
def call(system, user, temp=0.6):
    for attempt in range(6):
        try:
            ...
            wait = 5 * (attempt + 1)
            print(f"    ⚠️ 重试{attempt+1}/6: {str(e)[:50]} 等{wait}s", flush=True)
            time.sleep(wait)
    return None
...
for page in range(7, 16):
    ...
    if not os.path.exists(ocr_file):
        print(f"⏭️ 跳过第{page}页 (无OCR文件)", flush=True)
        continue
    raw = open(ocr_file, encoding='utf-8').read().strip()
    if len(raw) < 50:
        print(f"⏭️ 跳过第{page}页 (内容过少)", flush=True)
        continue
    
    print(f"🔄 [{time.strftime('%H:%M:%S')}] 第{page}页: 修复中...", flush=True)
    fixed = call(HEALER, raw, 0.3)
    if not fixed:
        print(f"❌ 第{page}页 修复失败", flush=True)
        continue
    
    print(f"   ✅ 修复完成 ({len(fixed)}字) 双语中...", flush=True)
    bilingual = call(ISAACSON, fixed, 0.8)
    if not bilingual:
        bilingual = fixed
    
    print(f"   ✅ 双语完成 萃取中...", flush=True)
    ...
    with open(f'output/preview/page_{page:03d}.md', 'w', encoding='utf-8') as f:
        f.write(f"# 第{page}页\n\n---\n\n{fixed}\n\n---\n\n{bilingual}\n\n---\n\n{cheat}\n")
    processed += 1
    print(f"✅ 第{page}页完成 → output/preview/page_{page:03d}.md (共{processed}页)", flush=True)

print(f"🎉 样章完成! 共处理 {processed} 页", flush=True)
